...
def inception_block(x_input, output_channel_for_each_path, name):
    """
    inception block implementation
    :param x_input:
    :param output_channel_for_each_path: eg: [10, 20, 5]
    :param name:
    :return:
    """
    # scope 防止命名冲突
    with tf.variable_scope(name):
        conv1_1 = tf.layers.conv2d(x_input, output_channel_for_each_path[0], (1, 1), strides=(1, 1), padding='same',
                                   activation=tf.nn.relu, name='conv1_1')
        conv3_3 = tf.layers.conv2d(x_input, output_channel_for_each_path[1], (3, 3), strides=(1, 1), padding='same',
                                   activation=tf.nn.relu, name='conv3_3')
        conv5_5 = tf.layers.conv2d(x_input, output_channel_for_each_path[2], (5, 5), strides=(1, 1), padding='same',
                                   activation=tf.nn.relu, name='conv5_5')
        max_pooling = tf.layers.max_pooling2d(x_input, (2, 2), (2, 2), name='max_pooling')
